...
print("🚀 ROBÔ 'NA MOSCA' LIGADO E RODANDO NA NUVEM 24H!")
print("⏱️ O sistema vai monitorar as partidas automaticamente a cada 10 minutos.\n")

while True:
    print(f"🔄 [{time.strftime('%H:%M:%S')}] Verificando a rodada ao vivo no Sofascore...")
    ...
    try:
        ...
        if jogos:
            ...
            for jogo in jogos:
                ...
                print(f"🎯 Novo jogo elegível encontrado: {time_casa} x {time_fora} ({minuto}')")
            ...
            if sinais_enviados_neste_ciclo > 0:
                print(f"✅ {sinais_enviados_neste_ciclo} novos sinais enviados!")
            else:
                print("💤 Nenhuma partida nova localizada agora.")
        else:
            print("⚠️ Sem jogos ao vivo no momento.")
